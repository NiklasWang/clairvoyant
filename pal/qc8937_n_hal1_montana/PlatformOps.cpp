...
void PlatformOps::init()
{
    ...
    if (NOTNULL(mHWI)) {
        ...
        mLastSharpenDenoiseParm.denoise = 0;
        mLastSharpenDenoiseParm.sharpen = 0;
    }
}
...
{
    return "Montana Qcom8937 Android N(7.0) Hal v1";
}
...
int32_t PlatformOps::setPlatformPrivSetting(PlatformPrivSet &parm)
{
    ...
    if (parm.denoise != mLastSharpenDenoiseParm.denoise ||
        parm.sharpen != mLastSharpenDenoiseParm.sharpen) {
        /** init asf, denoise params */
        bool enable;

        if (parm.denoise == 3 && parm.sharpen == 3){
            enable = true;
        }
        else{
            enable = false;
        ...
        Mutex::Autolock lock(mHWI->mParameters.mLock);
        mHWI->mParameters()->initBatchUpdate(mHWI->mParameters()->m_pParamBuf);
        mHWI->mParameters()->setSharpnessDenoisePP(enable);
        mHWI->mParameters()->commitParameters();
        mLastSharpenDenoiseParm = parm;
   }
...
int32_t PlatformOps::onCommandAvailable(int32_t cmd, int32_t arg1, int32_t arg2)
{
    ...
    mGetParms[GET_PARM_COMMAND_INF].commandInf.type =
        ...
        CommandInf::COMMAND_TYPE_MAX_INVALID;
    mGetParms[GET_PARM_COMMAND_INF].commandInf.type =
        cmd == CAMERA_CMD_LONGSHOT_OFF ?
