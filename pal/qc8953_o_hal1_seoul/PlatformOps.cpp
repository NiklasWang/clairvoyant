...
#define SEOUL_DP      40
#define TRULY_SENSOR_NAME  "ov13855_truly"
#define MAYANG_SENSOR_NAME "s5k3l8"
...
void PlatformOps::init()
{
    ...
    if (NOTNULL(mHWI)) {
        ...
        if (!strcmp(mSensorName, MAYANG_SENSOR_NAME)) {
            mGetParms[GET_PARM_CAM_TYPE].camType.type =
...
{
    return "Seoul Qcom8953 Android O(8.0) Hal v1";
}
    ...
    if (SUCCEED(rc)) {
        ...
        if(NOTNULL(val)){
            ...
            if (on) {
                ...
                if (!strcmp(moduleName, TRULY_SENSOR_NAME)) {
                    mGetParms[GET_PARM_DUAL_CAM_MODE].dualCamMode.funcMask |=
                        DUAL_CAMERA_MASK_STILL_BOKEH;
                } else if (!strcmp(moduleName, MAYANG_SENSOR_NAME)) {
