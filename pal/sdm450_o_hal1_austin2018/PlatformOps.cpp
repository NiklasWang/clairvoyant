...
#define SEOUL_DP      40
#define HLT_SENSOR_NAME  "ov16885_hlt_austin"
...
{
    return "Austin2018 SDM450 Android O(8.1.0) Hal v1";
}
...
int32_t PlatformOps::onMetadataAvailable(const void *dat)
{
...

    {
        ...
        if (metadata->is_tuning_params_valid) {
            ...
            mGetParms[GET_PARM_SUB_CAM_DATA].subCamData.auxRoiMapW =
                mHWI->mDualCamShareInfo.auxCropW;
            mGetParms[GET_PARM_SUB_CAM_DATA].subCamData.auxRoiMapH =
                mHWI->mDualCamShareInfo.auxCropH;
        ...
        IF_CMETA_AVAILABLE(cam_stream_crop_info_t, cppCropInfo,
            ...
            mGetParms[GET_PARM_SUB_CAM_DATA].subCamData.mainRoiMapW =
                cppCropInfo->roi_map.width;
            mGetParms[GET_PARM_SUB_CAM_DATA].subCamData.mainRoiMapH =
...
int32_t PlatformOps::onParameterAvailable(const void *data)
{
    ...
    if (SUCCEED(rc)) {
        ...
        const char *val = parm->get(QCameraParameters::KEY_QC_DUAL_CAMERA_MODE);
        if(NOTNULL(val)){
            int32_t on = !strcmp(val, QCameraParameters::VALUE_ON);
            if (on) {
                char *moduleName = mGetParms[GET_PARM_SUB_CAM_DATA].subCamData.mainModuleName;

                if (NOTNULL(moduleName) && !strcmp(moduleName, HLT_SENSOR_NAME)) {
                    /* Disable RT Bokeh to follow requirements
                    mGetParms[GET_PARM_DUAL_CAM_MODE].dualCamMode.funcMask |=
                        DUAL_CAMERA_MASK_RT_BOKEH;*/
                    mGetParms[GET_PARM_DUAL_CAM_MODE].dualCamMode.funcMask |=
                        DUAL_CAMERA_MASK_REFOCUS;
                    val = parm->get("dual-cam-night-shot");
                    if (NOTNULL(val) && !strcmp(val, "on")) {
                        mGetParms[GET_PARM_DUAL_CAM_MODE].dualCamMode.funcMask =
                            DUAL_CAMERA_MASK_NIGHT_SHOT;
                    }
                }
            }
