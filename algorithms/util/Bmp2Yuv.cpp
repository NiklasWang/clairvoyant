...
void showBmpHead(BITMAPFILEHEADER *pHead) {
    LOGI(module, "Bitmap header:");
    LOGI(module, "  type: 0x%x", pHead->bfType);
    LOGI(module, "  size: %d\n", pHead->bfSize);
    LOGI(module, "  reserved1: 0x%x", pHead->bfReserved1);
    LOGI(module, "  reserved2: 0x%x", pHead->bfReserved2);
    LOGI(module, "  data offset: %d", pHead->bfOffBits);
}

void showBmpInfoHead(BITMAPINFOHEADER *pInforHead) {
    LOGI(module, "Bitmap info");
    LOGI(module, "  struct size: %d", pInforHead->biSize);
    LOGI(module, "  width: %d", pInforHead->biWidth);
    LOGI(module, "  height: %d", pInforHead->biHeight);
    LOGI(module, "  planes: %d", pInforHead->biPlanes);
    LOGI(module, "  bits of color: %d", pInforHead->biBitCount);
    LOGI(module, "  compression type: %d", pInforHead->biCompression);
    LOGI(module, "  data sizes: %d", pInforHead->biSizeImage);
    LOGI(module, "  X pixels per meter: %d", pInforHead->biXPelsPerMeter);
    LOGI(module, "  Y pixels per meter: %d", pInforHead->biYPelsPerMeter);
    LOGI(module, "  number of colors in file: %d", pInforHead->biClrUsed);
    LOGI(module, "  number of import colors: %d", pInforHead->biClrImportant);
}
    ...
    uint8_t *orig, uint8_t *result, uint32_t w, uint32_t h)
{
    ...
    if (SUCCEED(rc)) {
        if (!makePalette(bmp, file, info, quad)) {
            LOGE(module, "Failed to find the rgb quad");
            rc = PARAM_INVALID;
        }
    }
    ...
    BITMAPINFOHEADER *info, uint8_t *rgbDataOut)
{
    ...
    if (SUCCEED(rc)) {
        ...
        if (SUCCEED(rc)) {
            ...
            LOGE(module, "Failed to get width and height");
        }
    }
    ...
    uint32_t w, uint32_t h, uint8_t *y, uint8_t *u, uint8_t *v)
{
    ...
    if (SUCCEED(rc)) {
        for (uint32_t i = 0, n = 0; n < w * h * 3; n += 3, i++) {
...
int32_t getBmpWidthHeight(uint8_t *bmp, uint32_t *w, uint32_t *h)
{
    ...
    if (SUCCEED(rc)) {
        ...
        if (file.bfType == BMP_FILE_INDENTIFIER) {
            ...
            rc = PARAM_INVALID;
            LOGE(module, "File is not a bmp file, %d", rc);
        }
    }
    ...
    if (SUCCEED(rc)) {
        ...
        if (!SUCCEED(rc)) {
            LOGE(module, "Failed to get bmp width and height. %d", rc);
        }
    }
...
int32_t Bmp2Yuv(uint8_t *bmp, uint8_t *yuv)
{
    ...
    uint32_t rgblen, ylen, ulen, vlen;
    uint32_t w, h;
    ...
    if (SUCCEED(rc)) {
        ...
        y = yuv;
        u = (uint8_t *)Malloc(ulen);
        v = (uint8_t *)Malloc(vlen);
        if (ISNULL(rgb) || ISNULL(y) || ISNULL(u) || ISNULL(v)) {
            LOGE(module, "Failed to alloc WxH %dx%d total %d"
                "bytes of rgb memory", w, h, rgblen + ulen + vlen);
            rc = NO_MEMORY;
        }
    }
    ...
    if (SUCCEED(rc)) {
        ...
        if (!SUCCEED(rc)) {
            LOGE(module, "Failed to parse rgb data");
        }
    }

    if (SUCCEED(rc)) {
        rc = covertRgb2Yuv(rgb, w, h, y, v, u);
        if (!SUCCEED(rc)) {
            LOGE(module, "Failed to convert rgb data to yuv");
        }
    }
...
int32_t Bmp2Yuv(char *bmp_path, char *yuv_path)
{
    ...
    if (SUCCEED(rc)) {
        ...
        if (!SUCCEED(rc)) {
            LOGE(module, "Failed to read bmp file %s, %d",
                bmp_path, rc);
        }
    }
    ...
    if (SUCCEED(rc)) {
        ...
        if (file.bfType == BMP_FILE_INDENTIFIER) {
            ...
            rc = PARAM_INVALID;
            LOGE(module, "File %s is not a bmp file, %d", bmp_path, rc);
        }
    }
    ...
    uint8_t *y, uint8_t *u, uint8_t *v, uint32_t size)
{
    ...
    if (SUCCEED(rc)) {
        ...
        if (written != length) {
            LOGE(module, "Failed to write %d bytes to %s", length, path);
            rc = NOT_READY;
        }
    }
